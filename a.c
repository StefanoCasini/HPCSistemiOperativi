...
int **alloca_inizializza_matrice(char *argv[], int dimensione)
{
    ...
    for (int i = 0; i < dimensione_risultato; i++)
        matrice[i] = malloc(dimensione_risultato * sizeof(int));
    for (int i = 0; i < dimensione_risultato; i++)
        matrice[i][0] = matrice[0][i] = 0;
    int c = 3;
    for (int i = 1; i < dimensione_risultato; i++)
    {
        for (int j = 1; j < dimensione_risultato; j++)
            matrice[i][j] = atoi(argv[c++]);
        c = 3 + dimensione * i;
    }
    return matrice;
}
...
void dividi_matrice(int **matrice, int i_inizio, int i_fine, int j_inizio, int j_fine)
{
    ...
    for (int i = 0; i < i_fine - i_inizio + 3; i++)
        divisa[i] = malloc((j_fine - j_inizio + 3) * sizeof(int));
...
int somma_intorno_scorrimento(int **matrice, int somma_prima, int i_prima, int j_prima, int i_dopo, int j_dopo)
{
    ...
    {
        for (int k = -1; k < 2; k++)
            somma -= matrice[i_prima + k][j_prima - 1 * direzione];
        for (int k = -1; k < 2; k++)
            somma += matrice[i_dopo + k][j_dopo + 1 * direzione];
    }
    else
    {
        for (int k = -1; k < 2; k++)
            somma -= matrice[i_prima - 1][j_prima + k];
        for (int k = -1; k < 2; k++)
            somma += matrice[i_dopo + 1][j_dopo + k];
    }
    return somma;
}
...
void media_intorno_processo(int **matrice, int righe, int colonne)
{
    ...
    for (int i = 1; i < righe - 1; i++)
    {
        if (i % 2 == 1)
            ...
            {
                if (i == 1 && j == 1)
                    somma = somma_intorno_prima(matrice, somma, i, j);
                else if (j == 1)
                    somma = somma_intorno_scorrimento(matrice, somma, i - 1, j, i, j);
                else
                    somma = somma_intorno_scorrimento(matrice, somma, i, j - 1, i, j);
